  ...
  return (
    <>
      ...
      <Button
        ...
        size={size}
        className={cn(
          "flex h-11 w-full items-center gap-2 border-border text-foreground hover:bg-muted/50",
          className,
        )}
        onClick={() => {
...
export function GoogleSignInDivider() {
  const { providers, loading } = useAuthProviders();
  if (loading || !providers.google) return null;
...
export function GoogleSignInButtonOnly({ className, size = "default" }: GoogleSignInButtonProps) {
  const { providers, loading } = useAuthProviders();
  if (loading || !providers.google) return null;
